...
//! environment. Tests are marked with #[ignore] and must be run manually with
//! live X11 session.
//!
//! # Requirements
//!
...
mod tests {
        ...
        F: FnOnce() -> T,
    {
        ...
        tracing::info!("{}: {:.2}ms", name, elapsed.as_secs_f64() * 1000.0);
        result
    }
    ...
    async fn test_list_windows_enumerate() {
        ...
        let windows = measure("list_windows", || {
            tokio::runtime::Handle::current()
                .block_on(backend.list_windows())
                .expect("list_windows failed")
        });
    ...
    async fn test_resolve_target_by_title() {
        ...
        // First, enumerate to find a real window
        let windows = backend.list_windows().await.expect("list_windows failed");

        if windows.is_empty() {
            tracing::warn!("Skipping: No windows found in X11 session");
            return;
        }

        // Try to resolve first window by title substring
        let first_window = &windows[0];
        let selector =
            WindowSelector::by_title(&first_window.title[..20.min(first_window.title.len())]);

        let handle = measure("resolve_target", || {
            tokio::runtime::Handle::current()
                .block_on(backend.resolve_target(&selector))
    ...
    async fn test_capture_window_first() {
        ...
        let image = measure("capture_window", || {
            tokio::runtime::Handle::current()
                .block_on(backend.capture_window(handle, &opts))
    ...
    async fn test_capture_display() {
        ...
        let opts = CaptureOptions::default();
        let image = measure("capture_display", || {
            tokio::runtime::Handle::current()
                .block_on(backend.capture_display(None, &opts))
    ...
    async fn test_capture_with_region() {
        ...
        let full_image = measure("capture_window_full", || {
            tokio::runtime::Handle::current()
                .block_on(backend.capture_window(handle.clone(), &full_opts))
        ...
        let region_image = measure("capture_window_with_region", || {
            tokio::runtime::Handle::current()
                .block_on(backend.capture_window(handle, &region_opts))
    ...
    async fn test_capture_with_scale() {
        ...
        let normal_image = measure("capture_window_normal_scale", || {
            tokio::runtime::Handle::current()
                .block_on(backend.capture_window(handle.clone(), &normal_opts))
        ...
        let scaled_image = measure("capture_window_with_scale", || {
            tokio::runtime::Handle::current()
                .block_on(backend.capture_window(handle, &scaled_opts))
